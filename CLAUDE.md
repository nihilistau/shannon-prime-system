...
**This is Shannon-Prime's math core. The canonical session bootstrap is `..\shannon-prime-lattice\prompt.md` — read it first** (project, current state, methodology, machine, doc map, operator). This file is the short version + this repo's specifics.
...
**Status (2026-06-17; engine-side, no math-core change):** XBAR P3 CLOSED end-to-end (P3.0→P3.4) + GNA "EAR" on silicon — previously recorded. **New this session (all engine-side; no math-core change):** **C2 Memo curator CLOSED** (autonomous Ring-2 recall loop above P3 — discrete bit-collision resolver 256-bit LSH / Hamming TAU_BITS=168 / r=256 + G-MEMO-{NULL,CUE,LOOP} GREEN on 12B; engine `tools/curator/`). **#222 CLOSED** (`gemma4_kv_replay` seam in `cuda_forward.cu` — SP_REPLAY into persistent gemma4_kv_* ABI, O(1) bit-exact rewind, G-222 GREEN). **Ring-3 Path A CLOSED** (VSA/HRR parameter-free gist consolidation, R3.1→R3.4 all GREEN, engine `tools/ring3/`; the real-domain VSA is host-numpy — the **Z_q/NTT native port using this repo's `core/ntt_crt` + `core/poly_ring` substrate is the next core task**). **G-XBAR-ORGANISM step 1 GREEN** (EAR→Ring-2 write seam, engine `6600cf4`). Detail: lattice `papers/CONTRACT-XBAR-P3` + `SESSION-HANDOFF.md`.
