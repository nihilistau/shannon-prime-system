 ...
 * read) lives in the Frobenius codec (sp/frobenius_lift.h: sp_frob_packed_tensor +
 * SP_FROB_ARENA_LAYOUT_VERSION). This module adds only the named collection: a set
 * of packed tensors built by iterating the model's weight tensors. Both the builder
 * (over a loaded GGUF-backed qwen3_model) and the .sp-model "adopt already-packed"
 * path live here; the engine and every backend consume the arena through this ABI.
 *
...
struct qwen3_model;   /* forward decl (sp/model.h) */
