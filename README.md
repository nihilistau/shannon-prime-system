...
**Update 2026-06-14 — KAIROS metal eviction (engine lane).** The C1-lite cold-evict (`T_GENKV_COLD_EVICT` here) now has a GPU twin in the engine: persistent-KV `gemma4_kv_rewind(Δ)` shears the decode position O(1), bit-exact (lattice CONTRACT-KAIROS-K0-K1 §5). The scalar-reference cold-evict in this repo remains the correctness anchor the engine's metal eviction is validated against.

**Update 2026-06-16 — KAIROS endurance + KAI-2 cloud lane (engine-side; status only).** The journaled-ring KAIROS loop (this repo's cold-evict anchor, deployed on the engine's persistent-KV ABI) passed a **6h unattended soak GREEN** on the dedicated 2060 (351 loops / ~8,400 ticks / 6h01m, 0 false / 0 missed / 0 pos-violation; the formal ≥24h gate is un-pursued by operator choice, not failed). The **KAI-2 latent-interrupt codec cloud lane is GREEN end-to-end** on the real bf16 gemma-4-12B; its pivot/selectivity gate (G-KAIROS-2) is **PENDING** (the pipeline trains + exports packets; the packet's pivot is unverified). No math-core change — recorded for cross-repo state. Detail: lattice CONTRACT-KAIROS-K0-K1 §5.9 / §6.4.
