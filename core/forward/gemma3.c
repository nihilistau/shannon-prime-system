/* gemma3.c — Gemma3 f32 reference forward pass (the math core's L1 Gemma3 forward).
 * Relocated out of the engine's src/forward/gemma3.c onto the migrated kernel stack
 * (forward_dispatch sp_matmul/sp_embed_row/sp_as_f32 + forward_kernels sp_rmsnorm/
 * sp_rmsnorm_head/sp_rope_neox/sp_attn_head); only the Gemma deltas vs Qwen3 live here.
 * Constants pinned from the oracle reference (lattice-llama gemma3.cpp + llama-hparams):
 *
 *   - Embedding scale: x *= sqrtf(n_embd) on the looked-up embedding.
 *   - RMSNorm: plain x/rms·w at every site — the (1+w) is baked into the GGUF weights,
 *     so the shared sp_rmsnorm is exact (adding 1 would double-count). Same for QK-norm.
 *   - QK-norm: sp_rmsnorm_head over head_dim, on Q and K, BEFORE RoPE. V is untouched.
 *   - RoPE: NEOX, base 1e6 (rope.freq_base) on GLOBAL layers, 10000 on LOCAL/SWA.
 *   - Layer pattern: a layer is GLOBAL (full causal, base 1e6) iff (il % 6 == 5), else
 *     LOCAL (sliding window 512, base 10000) — set_swa_pattern(6).
 *   - Query scale: 1/sqrt(head_dim) — the same ascale the kernels already apply.
 ...
 *   - No final-logit softcap. Tied LM head (output == token_embd). No overlay knobs.
 *
 * Faithful transcription: kernel calls renamed to the math-core ABI, includes repointed
 * from the engine's sp_engine tree to the sp tree. Definitive behavior-preservation is the
 * engine's M_GEMMA3_CPU regression, re-run against this implementation at the engine submodule bump.
 */
...
/* GELU, tanh approximation (Gemma FFN, LLM_FFN_GELU => ggml_gelu). The oracle evaluates
 * this through an F16 lookup table; the f32 closed form matches it to the §8.6.1 floor. */
