/* sp_hash_test.c — T_IO_HASH: the math-core hash primitives (sp/sp_hash.h) against
 * published test vectors -- the pure hash-vector leaf of FMT's E_FMT_0 gate. The
 * .sp-model header-layout half stays with the format-loader increment (it needs the
 * format structs that have not migrated into the math core yet). */
...
int main(void) { SP_RUN(HASH_VECTORS); return SP_DONE(); }
