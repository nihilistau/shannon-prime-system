/* arena.c — build the packed-weight arena (sp/arena.h). Per-row Frobenius
 * Q8/Q4 over the model's matmul weights, quantized once at load. Relocated
 * verbatim (arithmetic) out of the engine; the GGUF format knowledge stays
 * local (the row reader dequants source rows via sp_dequant_row), the byte
 * layout and the Frobenius packing are the math core's (sp/frobenius_lift.h). */
#include "sp/arena.h"
#include "sp/model.h"            /* qwen3_model / qwen3_layer / qwen3_config + gguf accessors */
#include "sp/frobenius_lift.h"
#include "sp/weight_dtype.h"     /* sp_dequant_row (GGML type ids share the SP_WDT_* encoding) */
 ...
 * to `cols` f32. Keeps the GGUF format knowledge here; the math-core packer never
 * sees a gguf_tensor and never materializes the whole tensor as f32. */
typedef struct {
