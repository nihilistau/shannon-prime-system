/* qwen25_fixture.c — see qwen25_fixture.h. Builds a tiny Qwen2.5-shaped .sp-model
 * with the full weight set sp_model_to_qwen25 reconstructs: per-layer QKV biases
 * (F32) instead of QK norms; no sandwich norms. */
...
#define FX_MAXT  64   /* 19 entries/layer × 2 layers + 3 global */
...
static int add_q8(tspec *T, int *n, const char *name, uint32_t rows, uint32_t cols, uint32_t salt) {
    ...
    for (uint32_t r = 0; r < rows; r++) sf[r] = 0.5f + (float)(r % 7u) * 0.05f;
    (*n)++;
    return 0;
}
...
static int add_f32(tspec *T, int *n, const char *name, uint32_t len) {
    ...
    for (uint32_t i = 0; i < len; i++) f[i] = 0.9f + (float)(i % 5u) * 0.02f;
    (*n)++;
    return 0;
}
...
int sp_qwen25_fixture_build(uint8_t **model_buf, uint8_t **tok_buf,
    ...
    a->arch_id = (uint32_t)SP_ARCH_ID_QWEN25; a->vocab_size = FX_V; a->hidden_dim = FX_E;
    ...
    a->ffn_variant = 0u; a->norm_variant = 0u; a->tied_embeddings = 1u; a->has_qk_norm = 0u;
    ...
    /* 12 tensors per layer: 7 Q8 pairs (14 entries) + 5 F32 (attn_norm, 3 biases, ffn_norm).
     * Plus global: token_embd (Q8 pair = 2) + output_norm (F32 = 1) = 3. Total for NL=2: 41. */
    tspec T[FX_MAXT];
    int n = 0, rc = 0;
    char nm[96];
    ...
    for (uint32_t L = 0; L < FX_NL && !rc; L++) {
        ...
        snprintf(nm, sizeof nm, "blk.%u.attn_q.bias",       L); rc |= add_f32(T, &n, nm, FX_NH*FX_HD);
        snprintf(nm, sizeof nm, "blk.%u.attn_k.bias",       L); rc |= add_f32(T, &n, nm, FX_NKV*FX_HD);
        snprintf(nm, sizeof nm, "blk.%u.attn_v.bias",       L); rc |= add_f32(T, &n, nm, FX_NKV*FX_HD);
