/* gemma3_fixture.c — see gemma3_fixture.h. Builds a tiny Gemma3-shaped .sp-model
 * with the full weight set sp_model_to_gemma3 reconstructs, including the two
 * sandwich-norm tensors per layer (post_attention_norm and post_ffw_norm). */
...
#define FX_MAXT  96      /* 13 tensor-pairs/layer × 2 layers + global tensors */
...
int sp_gemma3_fixture_build(uint8_t **model_buf, uint8_t **tok_buf,
    ...
    a->arch_id = (uint32_t)SP_ARCH_ID_GEMMA3; a->vocab_size = FX_V; a->hidden_dim = FX_E;
    a->n_layers = FX_NL; a->n_heads = FX_NH; a->n_kv_heads = FX_NKV; a->head_dim = FX_HD;
    a->max_context = 256u; a->swa_window = 512u; a->rope_freq_base = 1e6f;
    ...
    /* tensor specs: 13 tensors per layer (7 matmul ×2 + 6 f32 norms) + embd + output_norm */
    tspec T[FX_MAXT];
    int n = 0, rc = 0;
    char nm[96];
    ...
    for (uint32_t L = 0; L < FX_NL && !rc; L++) {
        ...
        snprintf(nm, sizeof nm, "blk.%u.post_attention_norm.weight",  L); rc |= add_f32(T, &n, nm, FX_E);
        ...
        snprintf(nm, sizeof nm, "blk.%u.post_ffw_norm.weight",       L); rc |= add_f32(T, &n, nm, FX_E);
    }
...

    {
        ...
        h.file_size = file_size; h.created_unix_seconds = 1700000001ull;
